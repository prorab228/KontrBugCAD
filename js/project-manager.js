...
class ProjectManager {
    ...
    serializeSketchElement(element) {
        ...
        if (element.material) {
            ...
            console.log('serializeMaterial', element.material);
        } else {
...
// СЕРИАЛИЗАЦИЯ МАТЕРИАЛА (с поддержкой LineBasicMaterial)
    serializeMaterial(material) {
        ...
        try {
            ...
            if (material.type === 'LineBasicMaterial') {
                ...
                if (material.linecap) matData.linecap = material.linecap;
                if (material.linejoin) matData.linejoin = material.linejoin;
                if (material.dashSize !== undefined) matData.dashSize = material.dashSize;
                if (material.gapSize !== undefined) matData.gapSize = material.gapSize;
                console.log('serializeMaterial LineDashedMaterial', matData);
            }
            else {
    ...
    deserializeSketchElement(data) {
        ...
        // Создаем материал с ОРИГИНАЛЬНЫМ цветом
        let material;
        const originalColor = data.userData?.originalColor ||
                             data.userData?.color ||
                             0x111111;

        if (data.material && data.material.type === 'LineDashedMaterial') {
            // Для пунктирных линий
            material = new THREE.LineDashedMaterial({
                color: originalColor,
                linewidth: data.material.linewidth || 2,
                dashSize: data.material.dashSize || 2,
                gapSize: data.material.gapSize || 2
            });
        } else {
            // Для обычных линий
        ...
        // Убеждаемся, что originalColor сохранен
        if (originalColor && !sketchObject.userData.originalColor) {
            sketchObject.userData.originalColor = originalColor;
        }

        // Восстанавливаем локальные точки
        ...
        sketchObject.userData.isSelected = false;

        // Для пунктирных линий вычисляем расстояния
        if (data.material?.type === 'LineDashedMaterial') {
            sketchObject.computeLineDistances();
        }
    ...
    deserializeMaterial(matData) {
        ...
        try {
            ...
            // Специфичные свойства для LineBasicMaterial dashSize gapSize
            ...
            if (material instanceof THREE.LineDashedMaterial) {
                ...
                if (matData.linecap !== undefined) material.linecap = matData.linecap;
                if (matData.linejoin !== undefined) material.linejoin = matData.linejoin;
                if (matData.dashSize !== undefined) material.dashSize = matData.dashSize;
                if (matData.gapSize !== undefined) material.gapSize = matData.gapSize;
            }
    ...
    // Восстановление скетча с детектированием контуров
    restoreSketch(sketchData) {
        ...
        // ВАЖНО: Детектируем контуры для восстановленного скетча
        if (this.editor.sketchManager && restoredElements > 0) {
            console.log("Detecting contours for restored sketch...");
            this.editor.sketchManager.detectContoursInSketch(plane);
        }
...
// ВОССТАНОВЛЕНИЕ ЭЛЕМЕНТА СКЕТЧА (переписанный метод)
    restoreSketchElement(plane, elementData) {
        ...
        try {
            ...
            // 5. Обновляем менеджер элементов
            if (this.editor.sketchManager && this.editor.sketchManager.elementManager) {
                // Убедимся, что элемент не дублируется
                const existingIndex = this.editor.sketchManager.elementManager.elements
                    .findIndex(el => el.mesh && el.mesh.uuid === element.uuid);
            ...
            // 6. Для текстовых элементов - создаем группу если нужно
