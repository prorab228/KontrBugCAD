...
class ExportSketchTool extends SketchToolBase {
    ...
    exportSketch(format) {
        if (!this.sketchManager.currentSketch || this.sketchManager.elements.length === 0) {
...
`;
        ...
        // Добавляем элементы
        this.sketchManager.elements.forEach((element, index) => {
...
EOF`;
    ...
    exportToJSON() {
        const sketchData = {
            ...
            elements: this.sketchManager.elements.map(element => ({
                type: element.type,
    ...
    exportToPDF() {
        ...
        this.sketchManager.elements.forEach(element => {
    ...
    calculateBoundingBox() {
        if (this.sketchManager.elements.length === 0) return null;
