...
class OvalSketchTool extends SketchToolBase {
    ...
    onMouseMove(e) {
        ...
        // Рассчитываем центр овала
        const centerX = this.tempElement.start.x + (dx / 2);
        const centerY = this.tempElement.start.y + (dy / 2);
        this.tempElement.center = new THREE.Vector3(centerX, centerY, 0);
    ...
    applyDimensions(values) {
        ...
        // Обновляем центр
        const centerX = this.tempElement.start.x + (this.tempElement.width * this.directionX / 2);
        const centerY = this.tempElement.start.y + (this.tempElement.height * this.directionY / 2);
        this.tempElement.center = new THREE.Vector3(centerX, centerY, 0);
