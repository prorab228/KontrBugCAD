...
//            }
    ...
    onDoubleClick(e) {
        ...
        if (intersects.length > 0) {
            const object = this.editor.objectsManager.findTopParent(intersects[0].object);
            console.log('doubleclick', object.userData.type);
            ...
            if (object.userData.type === 'sketch_plane') {
                   ...
                   // if (sketchTool) sketchTool.editExistingSketch(object);
                   this.editor.toolManager.setCurrentTool('sketch');
                   return true;
            }
            else this.editor.focusCameraOnObject(object);// Фокус камеры на объекте при двойном клике
